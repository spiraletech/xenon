...
public:
 ...
 [[nodiscard]] std::string_view name() const noexcept override; [[nodiscard]] ProviderCapabilities capabilities() const noexcept override; [[nodiscard]] RuntimeType runtime_type() const noexcept override;
 GenerationArtifact generate(const GenerationRequest&,const std::filesystem::path&) override; [[nodiscard]] BackendHealth health();
private: std::shared_ptr<IHttpTransport> http_; std::string base_url_; std::string checkpoint_path_;
};

class StableAudioBackend final : public IModelBackend {
public:
 StableAudioBackend(std::shared_ptr<IHttpTransport> http,std::string api_key={},std::string model="stable-audio-2.5");
 [[nodiscard]] std::string_view name() const noexcept override; [[nodiscard]] ProviderCapabilities capabilities() const noexcept override; [[nodiscard]] RuntimeType runtime_type() const noexcept override;
 GenerationArtifact generate(const GenerationRequest&,const std::filesystem::path&) override; [[nodiscard]] BackendHealth health() const;
private: std::shared_ptr<IHttpTransport> http_; std::string api_key_; std::string model_;
};
...
public:
 ...
 [[nodiscard]] SovereignRuntimeDiagnostics diagnose(); [[nodiscard]] std::unique_ptr<IModelBackend> make_ace_step() const; [[nodiscard]] std::unique_ptr<IModelBackend> make_stable_audio() const;
