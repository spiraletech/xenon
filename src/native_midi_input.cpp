...
struct NativeMidiInput::Impl {
    ...
    static void CALLBACK callback(HMIDIIN, UINT msg, DWORD_PTR instance, DWORD_PTR param1, DWORD_PTR param2) {
        ...
        if (self->messages.size() >= 4096)
            self->messages.erase(self->messages.begin(), self->messages.begin() + 1024);
...
void NativeMidiInput::open(unsigned int index) {
    ...
    HMIDIIN handle=nullptr;
    const auto result=midiInOpen(&handle,index,reinterpret_cast<DWORD_PTR>(&Impl::callback),reinterpret_cast<DWORD_PTR>(impl_.get()),CALLBACK_FUNCTION);
    ...
    if (midiInStart(handle) != MMSYSERR_NOERROR) {
        ...
        impl_->open=false;
        throw std::runtime_error("L41 failed to start MIDI input device");
    }
#else
    (void)index;
...
#ifdef _WIN32
    if (impl_->handle) {
        midiInStop(impl_->handle);
        midiInReset(impl_->handle);
        midiInClose(impl_->handle);
        impl_->handle=nullptr;
    }
#endif
    impl_->open=false;
