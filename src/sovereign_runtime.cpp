...
std::string find_json_string(const std::string& s,const std::string& key){const std::string k="\""+key+"\"";auto p=s.find(k);if(p==std::string::npos)return{};p=s.find(':',p+k.size());if(p==std::string::npos)return{};p=s.find('"',p);if(p==std::string::npos)return{};auto e=s.find('"',p+1);if(e==std::string::npos)return{};return s.substr(p+1,e-p-1);}
bool is_wav(const std::vector<std::byte>& b){return b.size()>=12&&static_cast<char>(b[0])=='R'&&static_cast<char>(b[1])=='I'&&static_cast<char>(b[2])=='F'&&static_cast<char>(b[3])=='F'&&static_cast<char>(b[8])=='W'&&static_cast<char>(b[9])=='A'&&static_cast<char>(b[10])=='V'&&static_cast<char>(b[11])=='E';}
#ifdef _WIN32
std::wstring widen(const std::string& s){if(s.empty())return{};const int n=MultiByteToWideChar(CP_UTF8,0,s.c_str(),static_cast<int>(s.size()),nullptr,0);if(n<=0)throw std::runtime_error("L41 UTF-8 conversion failed");std::wstring w(static_cast<std::size_t>(n),L'\0');MultiByteToWideChar(CP_UTF8,0,s.c_str(),static_cast<int>(s.size()),w.data(),n);return w;}
#endif
}

HttpResponse NativeHttpTransport::send(const HttpRequest& r){
#ifdef _WIN32
    const auto wurl=widen(r.url);URL_COMPONENTS uc{};uc.dwStructSize=sizeof(uc);uc.dwHostNameLength=static_cast<DWORD>(-1);uc.dwUrlPathLength=static_cast<DWORD>(-1);uc.dwExtraInfoLength=static_cast<DWORD>(-1);
    if(!WinHttpCrackUrl(wurl.c_str(),0,0,&uc))throw std::runtime_error("L41 WinHTTP invalid URL");
    std::wstring whost(uc.lpszHostName,uc.dwHostNameLength);std::wstring wpath(uc.lpszUrlPath?uc.lpszUrlPath:L"",uc.dwUrlPathLength);if(uc.lpszExtraInfo&&uc.dwExtraInfoLength)wpath.append(uc.lpszExtraInfo,uc.dwExtraInfoLength);const auto wmethod=widen(r.method);
    ...
    for(const auto& h:r.headers){std::wstring line=widen(h.first+": "+h.second);WinHttpAddRequestHeaders(req,line.c_str(),static_cast<DWORD>(-1),WINHTTP_ADDREQ_FLAG_ADD|WINHTTP_ADDREQ_FLAG_REPLACE);}if(!r.content_type.empty()){auto line=widen("Content-Type: "+r.content_type);WinHttpAddRequestHeaders(req,line.c_str(),static_cast<DWORD>(-1),WINHTTP_ADDREQ_FLAG_ADD|WINHTTP_ADDREQ_FLAG_REPLACE);}
    LPVOID ptr=r.body.empty()?WINHTTP_NO_REQUEST_DATA:reinterpret_cast<LPVOID>(const_cast<std::byte*>(r.body.data()));DWORD len=static_cast<DWORD>(r.body.size());
    ...
    HttpResponse out;out.status=static_cast<int>(status);for(;;){DWORD avail=0;if(!WinHttpQueryDataAvailable(req,&avail)||avail==0)break;auto old=out.body.size();out.body.resize(old+avail);DWORD read=0;if(!WinHttpReadData(req,out.body.data()+old,avail,&read))break;out.body.resize(old+read);}WinHttpCloseHandle(req);WinHttpCloseHandle(con);WinHttpCloseHandle(ses);return out;
#else
...
BackendHealth StableAudioBackend::health()const{BackendHealth h{"Stable Audio",!api_key_.empty(),false,{}};if(api_key_.empty()){h.detail="STABILITY_API_KEY missing";return h;}try{HttpRequest q{"GET","https://api.stability.ai/v1/user/account",{{"Authorization","Bearer "+api_key_},{"Accept","application/json"}}, {},{}};auto r=http_->send(q);h.reachable=r.status==200;h.detail=h.reachable?"authenticated account probe OK":"HTTP "+std::to_string(r.status);}catch(const std::exception&e){h.detail=e.what();}return h;}
GenerationArtifact StableAudioBackend::generate(const GenerationRequest&r,const std::filesystem::path& out){if(api_key_.empty())throw std::runtime_error("L41 STABILITY_API_KEY is not configured");if(model_!="stable-audio-2"&&model_!="stable-audio-2.5")throw std::runtime_error("L41 stable-audio-2 text endpoint supports model stable-audio-2 or stable-audio-2.5");std::filesystem::create_directories(out);const std::string boundary="xenon-l41-boundary";std::string body;body+=field("none","",boundary);body+=field("prompt",r.prompt,boundary);body+=field("output_format","wav",boundary);body+=field("duration",std::to_string(r.duration_seconds),boundary);body+=field("model",model_,boundary);body+=field("seed",std::to_string(r.seed),boundary);body+="--"+boundary+"--\r\n";HttpRequest q{"POST","https://api.stability.ai/v2beta/audio/stable-audio-2/text-to-audio",{{"Authorization","Bearer "+api_key_},{"Accept","audio/*"},{"stability-client-id","xenon-music-os"}},bytes(body),"multipart/form-data; boundary="+boundary};auto resp=http_->send(q);if(resp.status!=200)throw std::runtime_error("L41 Stable Audio HTTP "+std::to_string(resp.status)+": "+text(resp.body));if(!is_wav(resp.body))throw std::runtime_error("L41 Stable Audio returned non-WAV or corrupt audio data");auto p=out/("stable_audio_"+std::to_string(r.seed)+".wav");std::ofstream f(p,std::ios::binary);f.write(reinterpret_cast<const char*>(resp.body.data()),static_cast<std::streamsize>(resp.body.size()));if(!f)throw std::runtime_error("L41 failed to write Stable Audio result");GenerationArtifact a;a.audio_path=p;a.backend_name="Stable Audio";a.resolved_seed=r.seed;return a;}
