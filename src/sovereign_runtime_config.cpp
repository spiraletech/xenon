...
RuntimeConfig RuntimeConfig::load(const std::filesystem::path& path) {
    ...
    std::string header;std::getline(in, header);if (header != "XENON_RUNTIME|1") throw std::invalid_argument("L41 unsupported runtime config schema");
    RuntimeConfig c;if (!(in >> std::quoted(c.ace_url) >> std::quoted(c.ace_checkpoint) >> std::quoted(c.stability_model))) throw std::invalid_argument("L41 corrupt runtime config strings");
    ...
    std::size_t count=0;if (!(in >> count) || count > 1024) throw std::invalid_argument("L41 corrupt runtime plugin path count");
    for (std::size_t i=0;i<count;++i) {std::string value;if (!(in >> std::quoted(value))) throw std::invalid_argument("L41 corrupt runtime plugin path");c.plugin_paths.emplace_back(value);}return c;
}
...
std::size_t SovereignRuntime::configure_router(ModelRouter& router,const RuntimeConfig& config,int local_priority,int remote_priority) const {
    std::size_t count=0;
    if (!config.ace_checkpoint.empty()) { router.add_provider(std::make_unique<AceStepBackend>(http_,config.ace_url,config.ace_checkpoint),local_priority);++count; }
    if (!stability_key_.empty()) { router.add_provider(std::make_unique<StableAudioBackend>(http_,stability_key_,config.stability_model),remote_priority);++count; }
    return count;
}
