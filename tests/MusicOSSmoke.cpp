...
int main(){namespace fs=std::filesystem; xenon::ModelRouter router;router.add_provider(std::make_unique<xenon::NativePreviewBackend>(),100);xenon::MusicOS os(std::move(router));
 ...
 xenon::AutonomousProducerGoal goal;goal.project_id="l41-handoff";goal.goal="one short autonomous instrumental";goal.track_count=1;goal.duration_seconds=1;goal.max_revision_passes=0;goal.require_user_approval=true;auto produced=os.produce(goal,out/"autonomous");if(produced.tracks.size()!=1||!produced.awaiting_user_approval)return 8;
 fs::remove_all(out);std::cout<<"XENON L40/L41 Music OS unified surface verified\n";return 0;}
