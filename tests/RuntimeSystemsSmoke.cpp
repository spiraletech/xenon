...
int main(){
    ...
    if(!manager.loaded("fixture"))return 2;
    ...
    xenon::RuntimeJobControl cancelled;cancelled.cancel();
    ...
    manager.unload("fixture");if(manager.loaded("fixture"))return 6;
    ...
    if(sigil.device!="Sigil Guitar"||!sigil.gate||sigil.pressure<0.7)return 7;
    ...
    if(glyph.device!="GlyphPad"||glyph.y<=0.0||!glyph.gate)return 8;

    xenon::NativeMidiInput input;
    bool invalid_input=false;
    try{input.open(std::numeric_limits<unsigned int>::max());}catch(const std::out_of_range&){invalid_input=true;}
    if(!invalid_input||input.is_open())return 9;

#ifdef XENON_VST3_SDK_ENABLED
    if(!xenon::Vst3NativeHost::sdk_enabled())return 10;
#else
    if(xenon::Vst3NativeHost::sdk_enabled())return 11;
#endif
